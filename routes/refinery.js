...
// 1. GET PENDING SCRAP (Items available to be sent)
router.get('/pending-scrap', async (req, res) => {
    const { metal_type } = req.query; // 'GOLD' or 'SILVER'
    try {
...
// 2. CREATE BATCH (Send to Refinery)
    ...
    try {
        ...
        if (selected_item_ids && selected_item_ids.length > 0) {
            const itemsRes = await client.query(
                `SELECT SUM(net_weight) as total FROM old_metal_items WHERE id = ANY($1::int[])`,
                [selected_item_ids]
            );
        ...
        const batchRes = await client.query(
            `INSERT INTO refinery_batches 
            (batch_no, metal_type, status, gross_weight, items_count, sent_date)
        ...
        if (selected_item_ids && selected_item_ids.length > 0) {
            await client.query(
                `UPDATE old_metal_items SET status = 'BATCHED', batch_id = $1 
                 WHERE id = ANY($2::int[])`,
                [batchId, selected_item_ids]
            );
        }
...
// 4. RECEIVE REFINED GOLD (Update with Touch & Pure Wt)
    ...
    try {
        ...
        await pool.query(
            `UPDATE refinery_batches 
             SET status='REFINED', received_date=NOW(), 
                 refined_weight=$1, touch_percent=$2, pure_weight=$3, 
...
router.post('/use-stock', async (req, res) => {
    ...
    try {
        ...
        if (useWeight > available + 0.001) {
            throw new Error(`Insufficient Pure Weight. Available: ${available.toFixed(3)}`);
        }

        // A. PAY VENDOR (Reduce our debt to them)
        if (usage_type === 'PAY_VENDOR') {
            ...
            await client.query(
                ...
                (vendor_id, type, description, repaid_metal_weight, balance_after, metal_type, reference_id, reference_type)
                 VALUES ($1, 'REFINERY_PAYMENT', $2, $3, (SELECT balance_pure_weight FROM vendors WHERE id=$1), $4, $5, 'REFINERY_BATCH')`,
                [vendor_id, `Paid via Batch ${batch.batch_no}`, useWeight, batch.metal_type, batch_id]
            );
        }
        
        // B. ADD TO INVENTORY (As Raw Material)
        else if (usage_type === 'ADD_TO_INVENTORY') {
             const code = `REF-${Date.now()}`;
             await client.query(
                `INSERT INTO inventory_items 
                (item_name, barcode, metal_type, gross_weight, pure_weight, wastage_percent, source_type, status, stock_type)
                VALUES ($1, $2, $3, $4, $4, 100, 'REFINERY', 'AVAILABLE', 'RAW')`,
                [`Refined Pure ${batch.metal_type}`, code, batch.metal_type, useWeight]
             );
        }

        // Update Batch Used Amount
        const newUsed = currentUsed + useWeight;
        const newStatus = Math.abs(parseFloat(batch.pure_weight) - newUsed) < 0.01 ? 'COMPLETED' : 'REFINED';
        
        await client.query(
            "UPDATE refinery_batches SET used_weight = $1, status = $2 WHERE id = $3",
            [newUsed, newStatus, batch_id]
        );
